...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
       // set the camera in line with the player but keeping the z distance
        transform.position = new Vector3(TPlayer.position.x,TPlayer.position.y,transform.position.z);
         //Calculate and store the offset value by getting the distance between the player's position and camera's position.
        offset = transform.position - Gplayer.transform.position;
    }
    ...
    void LateUpdate()
    {
        ...
        transform.position = Gplayer.transform.position + offset;
    }
}